...
(function(factory) {
  ...
  function createNode(nodeData, level, opts) {
    ...
    // construct the content of node
	   ...
	   var $nodeDiv_sunny = $('<div' + (opts.draggable ? ' draggable="true"' : '') + (nodeData[opts.nodeId] ? ' id="' + nodeData[opts.nodeId] + '"' : '') + (nodeData.parentId ? ' data-parent="' + nodeData.parentId + '"' : '') + '> <div class="title col_box"> <label>' + nodeData[opts.nodeTitle] + '</label>' + typeof opts.nodeContent !== 'undefined' ? '<div class="content ">' + (nodeData[opts.nodeContent] || '') + '</div> </div>' : '') .addClass('node ' + (nodeData.className || '') +  (level >= opts.depth ? ' slide-up' : ''))
	 
	/* 
	 if(nodeData.className =="col_box manage"){
	...
	var $nodeDiv = $('<div' + (opts.draggable ? ' draggable="true"' : '') + (nodeData[opts.nodeId] ? ' id="' + nodeData[opts.nodeId] + '"' : '') + (nodeData.parentId ? ' data-parent="' + nodeData.parentId + '"' : '') + '> ')
      ...
      .append(typeof opts.nodeContent !== 'undefined' ? '<div class="content"><a href="#">' + nodeData[opts.nodeTitle] + '</a><a href="#">' + (nodeData[opts.nodeContent] || '') + '</a><a href="#" class="close_btn12"><img src="assets/images/cross.png" alt="close"/></a>' : '');
		 
	 }
	 
	 else {*/
	
  var commandTemplate = '<div class="content btn_cover1" id="content_'+ nodeData[opts.nodeId] +'"> <div class="status">' + (nodeData[opts.nodeContent] || '') + '</div><div class="dot_icon2"><div id="wave"> <span class="dot"></span> <span class="dot"></span> <span class="dot"></span> </div></div></div> <div class="treeHover" id="treeHover_'+ nodeData[opts.nodeId] +'">';

  if(nodeData['validated'] == 1){
    commandTemplate += '<a href="#" data-toggle="modal" data-backdrop = "static" data-target="#myValidateModal" id="validate_'+ nodeData[opts.nodeId] +'" onClick = "setModalTaskIdValidate('+nodeData[opts.nodeId]+')">VALIDATE</a>';
  }

  if(nodeData['applied_students'].length > 0){
    commandTemplate += '<a href="#" data-toggle="modal" data-backdrop = "static" data-target="#myAcceptModal" id="accept_'+ nodeData[opts.nodeId] +'" onClick = "setModalTaskIdAccept('+nodeData[opts.nodeId]+')">ACCEPT</a>';    
  }
  commandTemplate += '<a href="#" data-toggle="modal" data-backdrop = "static" data-target="#myModal1" id="edit_'+ nodeData[opts.nodeId] +'" onClick = "setModalTaskId('+nodeData[opts.nodeId]+')">EDIT</a> <a href="#" id="delete_'+ nodeData[opts.nodeId] +'">DELETE</a></div>';

	var $nodeDiv = $('<div' + (opts.draggable ? ' draggable="true"' : '') + (nodeData[opts.nodeId] ? ' id="' + nodeData[opts.nodeId] + '"' : '') + (nodeData.parentId ? ' data-parent="' + nodeData.parentId + '"' : '') + '> ')
      .addClass('node ' + (nodeData.className || '') +  (level >= opts.depth ? ' slide-up' : ''))
      .append('<div class="title" id="title_'+ nodeData[opts.nodeId] +'"> <label>' + nodeData[opts.nodeTitle] + '</label>')
      .append(typeof opts.nodeContent !== 'undefined' ? commandTemplate : '');
